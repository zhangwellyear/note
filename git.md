...
### 变基
假设有两个分支，变基前的示意图如下：
![变基前示意图](./assets/git/变基前.png)
若将`experiment`分支变基到`master`分支上，运行的命令如下：
```bash
git checkout experiment
git rebase master
```
变基的原理为：
1. 首先找到这两个分支（`experiment`和`master`）的共同祖先`C2`;
2. 对比当前分支`experiment`分支相对于祖先的历次提交，提取相应修改并存为临时文件;
3. 将当前分支指向目标基底`C3`;
4. 将临时文件中的修改依序应用到`C3`上。

#### 变基的优点
变基将一系列提交按照原有次序依次应用到另一分支，**使得提交历史更加整洁**

#### 变基的应用场景
向他人维护的项目贡献代码时，最后应将自己本地开发的代码变基到`origin/master`上，保证代码整洁，再向主项目提交修改，这样，项目维护者只要快进合并就好。

#### 变基的风险
提交位于自己所属仓库之外，别人可能会基于这些提交做开发，这种情况不要进行变基。

也就是说，不要拉取了远程的分支，然后在本地进行变基操作
